...
let
  ...
  wisprFlowFhs = profilePkgs.callPackage "${inputs.wispr-flow-linux}/nix/fhs.nix" {
...
assert lib.assertMsg (
  settings.binds."Mod+X".repeat == false
) "Wispr hands-free must be a one-shot binding";
assert lib.assertMsg (
  !builtins.any (
    rule: builtins.any (match: (match.app-id or "") == "^wispr-flow$") rule.matches
