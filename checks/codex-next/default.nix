{ pkgs, inputs, ... }:
let
  user = { name = "next-test"; size = "Min"; };
  configuration = (inputs.home-manager.lib.homeManagerConfiguration {
    inherit pkgs;
    extraSpecialArgs = { inherit user; };
    modules = [ ../../modules/home/profiles/min/codex-next.nix {
...
assert builtins.head service.Service.ExecStart == "${package}/bin/codex app-server --remote-control --listen unix:///home/next-test/.codex-next/app-server-control/app-server-control.sock";
