...
# - user:    one explicit entry from Horizon's users vector, selected by name
...
}:
{
  ...
  imports = [
    ...
    # Core checkup — generic timer; OS projection supplies capability and config.
    ./profiles/min/core-checkup.nix
    ./profiles/min/core-heartbeat.nix
