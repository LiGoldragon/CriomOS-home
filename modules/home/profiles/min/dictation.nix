...
mkIf (size.min && behavesAs.edge) {
  ...
  xdg.configFile."listener/environment.example".text = ''
    # Copy to ~/.config/listener/environment and set only real local commands.
    # Listener uses an honest not-configured transcript unless this is present.
    # LISTENER_TRANSCRIPTION_PROGRAM=/absolute/path/to/batch-transcriber
