...
in {
  options.criomosHome.coreCheckup = {
    ...
    configPath = mkOption { type = str; default = "/etc/core-checkup/roster.json"; description = "OS-projected endpoint and unit roster path."; };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services.core-checkup = {
      ...
      Service = {
        ...
        ExecStart = "${runner}/bin/core-checkup --roster ${cfg.configPath} --policy %h/.config/core-checkup/policy.json %S/core-checkup/events.ndjson %S/core-checkup/state.json";
      };
    };
